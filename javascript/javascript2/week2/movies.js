...
let shortMovieTitles = movieTitles.filter(title => title.length < 5);
//console.log(shortMovieTitles);

let longMovieTitles = movieTitles.filter(title => title.length >= 5);
//console.log(longMovieTitles);

let movieYear = movies.map(movies => movies.year);
//console.log(movieYear);

let eightiesMovies = movieYear.filter(year => (year <= 1989 && year >= 1980)) 
//console.log(eightiesMovies);

let moviesCount = eightiesMovies.length;
//console.log(moviesCount);

let moviesAddTag = movies.map(function(movies){
    if(movies.rating >= 7){
        return movies.tag = "good";
    }
    if(movies.rating >= 4 && movies.rating < 7){
        return movies.tag = "average";
    }
    if(movies.rating < 4){
        return movies.tag = "bad";
    }
}
)
//console.log(movies);
//console.log(moviesAddTag);

let moviesOver6 = movies
    .map(movies => movies.rating)
    .filter(rating => rating > 6);

//console.log(moviesOver6);


let filteredMovies = movieTitles
  .map(titles => titles.toLowerCase())
  .filter(movie => movie.includes('surfer') || movie.includes('alien') || movie.includes('benjamin'))
  .length;

//console.log(filteredMovies);

//find duplicated words in the titles of each movie. still working on this task

  words = movieTitles.map(movie => movie.split(" "))
